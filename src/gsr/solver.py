...
def viewpoint_localizer(viewpoint, gaussians, base_lr: float=1e-3, use_variance: bool=True, tau_registration: float=10.0, w_limit: float=3.0): # True    False
    ...
    Args:
        ...
        tau_registration (float, optional). Defaults to 10.0.
        w_limit (float, optional). Defaults to 3.0.
...
def gaussian_registration(src_dict, tgt_dict, config: dict, visualize=False, config_var: dict=None):
    ...
    """
    tau_registration = config_var["tau_registration"]
    w_limit = config_var["w_limit"]
    ...
    for viewpoint in src_view_list:
        ...
        if config["use_render"]:
            ...
            viewpoint.load_rgb()
        converged, pred_tsfm, residual, loss_log = viewpoint_localizer(viewpoint, tgt_3dgs, config["base_lr"], tau_registration, w_limit)
    ...
    for viewpoint in tgt_view_list:
        if config["use_render"]:
            ...
            viewpoint.load_rgb()
        converged, pred_tsfm, residual, loss_log = viewpoint_localizer(viewpoint, src_3dgs, config["base_lr"], tau_registration, w_limit)
